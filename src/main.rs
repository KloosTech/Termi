...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::Sonarr { query, url, api_key } => {
            ...
            let selection = Select::with_theme(&ColorfulTheme::default())
                .with_prompt(format!("Results for \"{}\" — select to add", output.corrected_query))
                .items(&labels)
                .interact_opt()
        ...
        Command::Radarr { query, url, api_key } => {
            let query_str = query.join(" ");
        ...
        Command::Lidarr { query, url, api_key } => {
            let query_str = query.join(" ");
